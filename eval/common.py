...
def load_sentences(n: int | None = None) -> list[str]:
    lines = [l.strip() for l in (ROOT / "sentences_de.txt").read_text(encoding="utf-8").splitlines()]
...
def analyse_cached(text: str, wav_path: Path, json_path: Path) -> dict:
    ...
    rep = analyse(text, str(wav_path), recognizer())
...
def canonical_tokens_by_word(text: str) -> list[tuple[str, list[str]]]:
    from mdd.g2p import text_to_ipa_words
    from mdd.normalize import tokenize
    return [(w, tokenize(ipa)) for w, ipa in text_to_ipa_words(text)]
